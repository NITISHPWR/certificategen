...
router.post('/PreviewL', async (req, res) => {
    try {
	 const { templateId, dynamicData } = req.body;
	console.log("==== "+templateId);
        // Fetch template data from the database based on tid
        if (templateId == "" || templateId == undefined) {
            return res.status(404).json({ message: 'Invalid TemplateId' });
        }

        const templateData = await BlankCertSchema.findOne({ tid: templateId }).exec();
        if (!templateData) {
        ...
        const pdfPath = path.join(__dirname, '..', templateId+'preview.pdf');
        ...
        // Send the PDF file as a downloadable response
            res.download(pdfPath, templateId+'preview.pdf', (err) => {
                if (err) {
                    console.error(err);
