...
export default function HeaderTextboxP(props) {
  const [headerText, setHeaderText] = useState(""); // Initialize with an empty string
  ...
  useEffect(() => {
    ...
    if (inputRef.current) {
      inputRef.current.style.width = `${headerText.length * 8 + 60}px`; // Adjust the factor (10) and initial width (100) based on your preference
